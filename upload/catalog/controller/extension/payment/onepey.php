...
class ControllerExtensionPaymentOnePEY extends Controller {
  ...
  public function index() {
    ...
    $this->language->load('extension/payment/onepey');
    ...
    $callback_url = $this->url->link('extension/payment/OnePEY/callback', '', 'SSL');
    $return_url = $this->url->link('extension/payment/OnePEY/customer_return', '', 'SSL');
    
    $data['action'] = 'https://' . $this->config->get('payment_onepey_domain_payment_page') . '/transaction/customer';
    ...
    $redirectRequest = array(
	    array('paramName' => 'merchantID' , 'paramValue' => $this->config->get('payment_onepey_companyid')),
    ...
    $pSign = $this->config->get('payment_onepey_encyptionkey');
    foreach ($redirectRequest as $param){
    	$pSign .= $param['paramValue'];
    }
    $redirectRequest[] = array('paramName' => 'pSign', 'paramValue' => hash($this->config->get('payment_onepey_encyptionalgo'), $pSign));
    ...
    return $this->load->view('extension/payment/onepey', $data);
  }
  ...
  public function generateToken(){
    ...
    $request = array(
    		'merchantID' => $this->config->get('payment_onepey_companyid'),
    		'amount' => $orderAmount,
    ...
    $request['pSign'] = hash($this->config->get('payment_onepey_encyptionalgo'), $pSign);
    ...
    $ctp_url = 'https://' . $this->config->get('payment_onepey_domain_payment_page') . '/transaction/execute';
  ...
  private function processOrder($response){
  	...
  	if ($order_info && $status != null) {
  		...
  		if($status == self::APPROVED){
  			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_onepey_completed_status_id'), "UID: {$transaction_id}. Message: {$transaction_message}", true);
  			return true;
  		}
  		else if(in_array($status, array(self::FAILED, self::DECLINED))){
  			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_onepey_failed_status_id'), "UID: {$transaction_id}. Message: {$transaction_message}", true);
  		}
